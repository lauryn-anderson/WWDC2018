...
class AnswersViewController: UITableViewController, PlaygroundLiveViewMessageHandler, InputCellDelegate {
    ...
    override func viewDidLoad() {
        ...
        placeholderView.backgroundColor = UIColor(white: 1.0, alpha: 0.02)
        let titleLabel = UILabel()
        titleLabel.text = NSLocalizedString("Nothing is happening right now", comment: "Title label when there is no output from the answers template")
        ...
        messageLabel.text = NSLocalizedString("When you are ready, run your code to get things moving", comment: "Content label when there is no output from the answers template")
